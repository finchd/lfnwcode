...
# Login into azure using SPN
	...
	# Code to capture admin username
        admin_username=$(az acs show -n $Servicename -g $Resource | jq -r '.linuxProfile.adminUsername')
		echo "Successfully captured your Admin Username: $admin_username"
...
# Create SSH Tunnel and check to ensure tunnel is successfully created, if errors, try again up to 5 times
	echo "Opening SSH tunnel to ACS..."
		...
		until [ $n -ge 5 ]
		do
			ssh -fNL $local_port:localhost:$remote_port -p 2200 $admin_username@$master_fqdn -i /root/.ssh/lfnw -o StrictHostKeyChecking=no -o ServerAliveInterval=240 &>/dev/null && echo "ACS SSH Tunnel successfully opened..." && break
